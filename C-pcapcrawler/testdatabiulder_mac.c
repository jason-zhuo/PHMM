	...
	if(fgets(line, 100, fr) != NULL)
	  {
		...
		for(round=0;round<10;round++){ 
			...
			sprintf(tcpdump,"tcpdump -i en0 tcp and port 1984 and host 146.57.249.111 -w %s/%d-%d.pcap &",currentdir,num,round);
					//printf("%s\n",tcpdump);			
					system(tcpdump);                                       	
				  sprintf(firefox,"/Applications/Firefox.app/Contents/MacOS/firefox -P default %s &",mainpage);  // vist Mainpage first
          printf("%s\n", firefox);
          system(firefox);
          sleep(2);
          ...
          //printf("%s\n", name);
          sprintf(firefox,"/Applications/Firefox.app/Contents/MacOS/firefox -P zzl %s &",tmp);  // vist second random page
          printf("%s\n", firefox);
