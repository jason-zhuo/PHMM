...
rootdir = r"/Users/zhuozhongliu/Desktop/KPFretest/Alexa30(2)/full/"
alexa_monitored_data = rootdir + r"Train/"
#alexa_monitored_data = rootdir +r"Google15_size/"   # for google
...
def dictionary_(path_to_dict = dic_of_feature_data, path_to_alexa = alexa_monitored_data, path_to_hs = hs_monitored_data, path_to_unmon = unmonitored_data,
    ...
    for i in range(100):
        for j in range(100):
            fname = str(i) + "_" + str(j)
            if os.path.exists(pathtoTest + fname):
                print i
                tcp_dump = open(pathtoTest + fname).readlines()
                g = []
                g.append(RF_fextract.TOTAL_FEATURES(tcp_dump))
                data_dict['mytest_feature'].append(g)
                data_dict['mytest_label'].append((i, j))
    ...
    assert len(data_dict['mytest_feature']) == len(data_dict['mytest_label'])
...
def mon_train_test_references(mon_type, path_to_dict = dic_of_feature_data):
    ...
    test_data=list(dic['mytest_feature'])
    test_label = list(dic['mytest_label'])
    flat_train_data = []
    flat_test_data = []
    ...
    for i in range(100):
        for j in range(100):
            ...
            if os.path.exists(pathtoTest + fname):
                ...
                g = []
                g.append(RF_fextract.TOTAL_FEATURES(tcp_dump))
                flat_test_data.append(g)
                test_label.append((i, j))
...
def RF_closedworld(mon_type, path_to_dict = dic_of_feature_data):
    ...
    confusematrix=metrics.confusion_matrix(te_label,model.predict(te_data))
    te_predict = model.predict(te_data)
    cm = metrics.confusion_matrix(list(te_label), te_predict)
    cmnparray = np.array(cm)
    TP = np.sum(cmnparray.diagonal())

    print("TP: " + str(TP))
    print("precision: " + str(metrics.precision_score(list(te_label), te_predict)))
    print("Recall: " + str(metrics.recall_score(list(te_label), te_predict)))
    print("f1_score: " + str(metrics.f1_score(list(te_label), te_predict)))
    #ht.plotHeatmap("","","",np.asarray(confusematrix))
    # print(confusematrix)
